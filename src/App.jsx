...
export default function App() {
  ...
  useEffect(() => {
    ...
    const blocked = publicData.filter(p => 
      userProfile.blockedUsers?.includes(p.userId)
    );
    
    setFriendsList(friends);
    setFollowersList(followers);
    setBlockedUsersList(blocked);
  }, [publicData, userProfile.following, userProfile.blockedUsers, user?.uid]);
  ...
  */
  ...
  const blockUser = async (userId, userName) => {
    ...
    // Agregar a la lista de usuarios bloqueados
    await updateDoc(doc(db, 'profiles', user.uid), { 
      blockedUsers: arrayUnion(userId)
    });
    
    // Eliminar de la lista de seguimiento si está
    await updateDoc(doc(db, 'profiles', user.uid), { 
      following: arrayRemove(userId)
    });
    
    // Eliminar de los seguidores del usuario bloqueado
    await updateDoc(doc(db, 'profiles', userId), { 
      following: arrayRemove(user.uid),
      followersCount: increment(-1)
    });
    
    alert(lang === 'es' ? `Usuario ${userName} bloqueado` : `User ${userName} blocked`);
  };

  const unblockUser = async (userId, userName) => {
    if (!user) return;
    
    // Eliminar de la lista de usuarios bloqueados
    await updateDoc(doc(db, 'profiles', user.uid), { 
      blockedUsers: arrayRemove(userId)
    });
    
    alert(lang === 'es' ? `Usuario ${userName} desbloqueado` : `User ${userName} unblocked`);
  };
  ...
  // Filtrar usuarios para la sección de amigos
  const filteredUsers = publicData.filter(p => {
    if (p.userId === user?.uid) return false; // Excluir al usuario actual
    if (userProfile.blockedUsers?.includes(p.userId)) return false; // Excluir usuarios bloqueados
    if (userProfile.dismissedUsers?.includes(p.userId)) return false; // Excluir usuarios ocultos
                  ...
                  {showFriendsSection ? (
                    <div className="space-y-6 animate-in fade-in">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className={`text-center p-4 rounded-2xl ${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-slate-50'}`}>
                          <p className="text-2xl font-black text-red-600 dark:text-red-400">{blockedUsersList.length}</p>
                          <p className="text-[10px] font-black uppercase text-slate-400 dark:text-gray-400">{t.blocked_users}</p>
                        </div>
                      </div>
                        ...
                        {filteredUsers.length === 0 ? (
                          ...
                          <div className="space-y-3 max-h-96 overflow-y-auto">
                            {filteredUsers.map(p => {
                              ...
                              const isBlocked = userProfile.blockedUsers?.includes(p.userId);
                              ...
                              return (
                                <div key={p.userId} className={`${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-white'} p-4 rounded-2xl border ${themeClasses.border} flex items-center justify-between`}>
                                  ...
                                  <div className="flex gap-2">
                                    {isBlocked ? (
                                      <button 
                                        onClick={() => unblockUser(p.userId, p.name)}
                                      ...
                                      <>
                                        ...
                                        <button 
                                          onClick={() => blockUser(p.userId, p.name)}
