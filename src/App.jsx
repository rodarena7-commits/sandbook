...
const i18n = {
  es: {
    ...
    // TRADUCCIONES PARA EL PLAN DE LECTURA MEJORADO
  ...
  en: {
    ...
    // TRANSLATIONS FOR IMPROVED READING PLAN
    days_left: "days left",
    my_notes: "My notes",
...
export default function App() {
  ...
  // Estados para estadísticas de usuario
  ...
  // --- NUEVOS ESTADOS PARA VISUALIZACIÓN DE INTERACCIONES DE USUARIO CON LIBROS ---
  const [usersWhoRead, setUsersWhoRead] = useState([]); // Usuarios que han leído el libro
  const [usersWhoFavorited, setUsersWhoFavorited] = useState([]); // Usuarios que lo tienen como favorito
  const [usersWhoLiked, setUsersWhoLiked] = useState([]); // Usuarios que dieron like
  const [usersWhoDisliked, setUsersWhoDisliked] = useState([]); // Usuarios que dieron dislike
  const [usersWithBook, setUsersWithBook] = useState([]); // Usuarios que lo tienen en biblioteca
  const [showUserListModal, setShowUserListModal] = useState(false); // Control para el modal de lista de usuarios
  const [userListTitle, setUserListTitle] = useState(''); // Título del modal
  const [userListData, setUserListData] = useState([]); // Datos de los usuarios a mostrar

  // --- NUEVOS ESTADOS PARA PLAN DE LECTURA MEJORADO ---
  const [checkpointNotes, setCheckpointNotes] = useState({}); // Para almacenar notas temporales antes de guardar
  ...
  // --- NUEVA FUNCIÓN PARA CARGAR LAS INTERACCIONES DE USUARIOS CON UN LIBRO ---
  const loadBookUserInteractions = async (bookId) => {
    if (!bookId) return;
    
    try {
      // Usuarios que lo tienen en su biblioteca (status library o inLibrary true)
      ...
      // Usuarios que dieron like/dislike (desde globalLikes)
  ...
  // --- FUNCIÓN PARA AGREGAR LIBRO CON ESTADÍSTICAS ---
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} pb-24 font-sans overflow-x-hidden selection:bg-indigo-200 selection:text-white`}>
            ...
            <div className="flex-shrink-0 px-6 pb-4">
              ...
              <div className="text-center">
                ...
                <div className="grid grid-cols-3 gap-4 mt-4">
                  ...
                  <div className="text-center">
                    <p className="font-black text-lg">{userProfileModalData.libraryCount || 0}</p>
              ...
              <div className="mt-6">
                ...
                {selectedUserBooks.length === 0 ? (
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {filteredExternalBooks.slice(0, 8).map((book, idx) => (
                      <div 
                        key={idx} 
                        className="aspect-[2/3] rounded-xl overflow-hidden cursor-pointer hover:scale-105 transition-all relative group"
                        onClick={() => {
      ...
      {/* MODAL DETALLE DE LIBRO (MEJORADO) - AHORA CON SCROLL Y ZOOM ELIMINADO */}
            ...
            <div className="flex-shrink-0 px-6 pb-4">
              <div className="relative -mt-24 mb-4 flex justify-center">
                <img 
                  ...
                  onClick={() => openBookDetailModal(bookDetailData)} // Simplemente volver a abrir el mismo modal (no hace zoom)
                />
              </div>
              <div className="text-center">
                ...
                {/* Estadísticas del libro - AHORA SON CLICABLES PARA VER USUARIOS */}
            ...
            <div className="flex-shrink-0 px-6 pb-4">
              ...
              <div className="text-center">
                ...
                <div className="grid grid-cols-3 gap-4 mt-4">
                  ...
                  <div className="text-center">
                    <p className="font-black text-lg">{selectedUserProfile.libraryCount || 0}</p>
                    <p className="text-[10px] text-slate-400 uppercase">{t.in_library}</p>
                  </div>
                </div>
              </div>
                    ...
                    <div key={book.bookId} className={`${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-slate-50'} p-4 rounded-2xl border ${themeClasses.border}`}>
                      <div className="flex gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* NUEVO BOTÓN DE PRÉSTAMO PARA LIBROS DE OTROS USUARIOS */}
            ...
            <div className="flex-shrink-0 px-6 pb-4">
              <div className="relative -mt-16 mb-4">
                <img 
                  ...
                  onClick={() => openBookDetailModal(viewingBook)} // Redirige al modal mejorado de detalle
                />
              </div>
              <div className="text-center">
      ...
      <header className={`fixed top-0 left-0 right-0 z-[100] ${themeClasses.card} shadow-lg border-b ${themeClasses.border}`}>
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {/* FOTO DE PERFIL - CÍRCULO PERFECTO */}
            <img 
                      ...
                      <div key={book.id} className={`${themeClasses.card} rounded-3xl p-4 shadow-md border ${themeClasses.border}`}>
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {/* NUEVOS BOTONES DE LIKE Y DISLIKE EN RESULTADOS DE BÚSQUEDA */}
