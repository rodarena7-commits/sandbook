...
import { 
  BookOpen, Search, Trophy, Plus, CheckCircle, Layout, User, Award, Loader2, PenTool, Globe, Camera, MessageSquare, Send, X, ChevronDown, ChevronUp, Settings, Edit3, ListChecks, Lock, Flag, Sparkles, Star, Upload, Book as BookIcon, AlertCircle, Calendar, FileText, Info, Maximize2, Minimize2, UserPlus, UserCheck, Users, Trash2, Facebook, Languages, Share2, UserX, MessageCircle, StickyNote, Barcode, Library
} from 'lucide-react';
...
const i18n = {
  es: {
    ...
    invite: "INVITAR", read: "Leídos", pending: "Pendientes", favorites: "⭐",
  ...
  en: {
    ...
    invite: "INVITE", read: "Read", pending: "Pending", favorites: "⭐",
...
export default function App() {
  ...
  const submitGlobalReview = async () => {
    if (!userRating || !viewingBook) return;
    const bookId = viewingBook.id || viewingBook.bookId;
    await addDoc(collection(db, 'comments'), {
      bookId, userId: user.uid, userName: userProfile.name, userPic: userProfile.profilePic, text: userComment, rating: userRating, timestamp: serverTimestamp()
    });
    setUserComment("");
    setUserRating(0);
  };
    ...
    <div className="min-h-screen bg-slate-50 pb-24 font-sans text-slate-900 overflow-x-hidden selection:bg-indigo-100">
        ...
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
           <div className="bg-white w-full max-w-lg h-[85vh] rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95">
              <div className="relative h-48 bg-indigo-600 flex-shrink-0">
                 ...
                 <button onClick={() => setViewingBook(null)} className="absolute top-6 right-6 p-2 bg-white/20 text-white rounded-full"><X size={24}/></button>
              </div>
        ...
        <div className="fixed inset-0 z-[400] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 text-center shadow-2xl">
            ...
            <div className="flex gap-3">
              <button onClick={() => setBookToDelete(null)} className="flex-1 py-4 rounded-2xl bg-slate-100 font-bold text-slate-500 uppercase text-[10px]">{t.cancel}</button>
              <button onClick={async () => { await deleteDoc(doc(db, 'users', user.uid, 'myBooks', bookToDelete)); setBookToDelete(null); }} className="flex-1 py-4 rounded-2xl bg-red-500 text-white font-black uppercase text-[10px]">{t.delete_btn}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {planningBook && (
        <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-[2.5rem] p-8 shadow-2xl animate-in zoom-in-95">
            <h3 className="font-black text-xl mb-6 text-indigo-600 flex items-center gap-2 uppercase tracking-tighter"><Calendar /> {t.plan}</h3>
            <div className="space-y-4">
               <div><label className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">{t.manual_p}</label>
               <input type="number" value={manualPages} onChange={(e) => setManualPages(e.target.value)} className="w-full bg-slate-50 border rounded-2xl p-4 font-bold outline-none border-slate-100 focus:ring-2 focus:ring-indigo-500" /></div>
               <div><label className="text-[10px] font-black text-slate-400 uppercase ml-1 tracking-widest">{t.days}</label>
               <input type="number" value={planDays} onChange={(e) => setPlanDays(e.target.value)} className="w-full bg-slate-50 border rounded-2xl p-4 font-bold outline-none border-slate-100 focus:ring-2 focus:ring-indigo-500" /></div>
               <button onClick={saveReadingPlan} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg mt-4 tracking-widest active:scale-95 transition-all">{t.start}</button>
               <button onClick={() => setPlanningBook(null)} className="w-full text-slate-400 font-bold text-[10px] mt-3 uppercase tracking-widest">{t.cancel}</button>
            </div>
          </div>
        </div>
      )}
            ...
            <div className="space-y-4">
              {filteredBooks.length === 0 ? (
                 <div className="text-center py-20 bg-white rounded-[2.5rem] border border-dashed border-slate-200">
                    <BookIcon className="mx-auto text-slate-200 mb-4" size={48} />
                    <p className="text-slate-400 font-bold uppercase text-[10px] tracking-widest">Sin libros en esta sección</p>
                 </div>
                ...
                return (
                  <div key={i} className="bg-white p-4 rounded-[2.5rem] border border-slate-100 shadow-sm animate-in fade-in overflow-hidden">
                        ...
                        {book.checkpoints.map((cp, idx) => (
                          <div key={idx} className="space-y-2">
                             ...
                             <div className="flex items-start gap-2 px-2">
                                ...
                                <input className="flex-1 bg-transparent border-none text-[10px] outline-none text-slate-400 placeholder:opacity-30" placeholder={t.daily_notes} value={cp.note || ""}
                                   onChange={async (e) => {
            ...
            <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                   <input type="text" placeholder={t.search_p} className="w-full pl-12 pr-4 py-4 bg-slate-50 border rounded-[1.5rem] outline-none font-medium text-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && performSearch()} />
                   <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18} />
                </div>
                <button onClick={startScanner} className="bg-indigo-100 text-indigo-600 p-4 rounded-[1.25rem] active:scale-95 transition-all shadow-inner"><Barcode size={24} /></button>
              </div>
           ...
           <div className="space-y-6 animate-in fade-in">
              <div className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-sm">
                ...
                <div className="relative mb-6">
                   <input 
                      ...
                      className="w-full pl-12 pr-4 py-4 bg-slate-50 border rounded-2xl outline-none font-medium text-sm border-slate-100 focus:ring-2 focus:ring-indigo-500"
                      value={profileSearch}
              ...
              <div className="grid grid-cols-1 gap-3">
                 {publicData
                   ...
                   .map(p => (
                    <div key={p.userId} className="bg-white p-4 rounded-[2.5rem] border border-slate-100 shadow-sm flex items-center justify-between animate-in zoom-in-95 group">
                       <div className="flex items-center gap-3">
        ...
        {activeTab === 'profile' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm text-center relative overflow-hidden">
              <button onClick={() => setIsEditingProfile(true)} className="absolute top-8 right-8 p-2 bg-slate-50 rounded-full text-slate-400 hover:text-indigo-600 shadow-sm transition-all"><Edit3 size={18} /></button>
              ...
              <div className="flex justify-center gap-8 text-[9px] font-black text-slate-400 uppercase bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 shadow-inner">
                 <div className="text-center"><span className="text-indigo-600 text-xl block leading-none mb-1">{userProfile.readCount}</span>{t.read}</div>
                 <div className="w-px h-8 bg-slate-200"></div>
                 <div className="text-center"><span className="text-purple-600 text-xl block leading-none mb-1">{userProfile.followersCount}</span>Fans</div>
                 <div className="w-px h-8 bg-slate-200"></div>
                 <div className="text-center"><span className="text-amber-600 text-xl block leading-none mb-1">{userProfile.badges?.length || 0}</span>Logros</div>
              </div>
            </div>

            {isEditingProfile && (
              <div className="bg-white p-6 rounded-[3rem] border-2 border-indigo-500 shadow-2xl space-y-4 animate-in slide-in-from-top-4">
                <h3 className="font-black text-sm uppercase flex items-center gap-2 mb-2"><Settings size={18}/> Editar Perfil</h3>
                ...
                <label className="w-full flex items-center justify-center gap-2 bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl py-6 cursor-pointer text-slate-400 hover:border-indigo-300 transition-all">
                  <Upload size={18} /> <span className="text-xs font-black uppercase">Cambiar Foto</span>
                  <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
                </label>
                <button onClick={async () => { await updateDoc(doc(db, 'profiles', user.uid), { name: userProfile.name, profilePic: userProfile.profilePic }); setIsEditingProfile(false); }} className="w-full bg-indigo-600 text-white py-4 rounded-2xl font-black text-xs uppercase shadow-lg active:scale-95 transition-all">Guardar Cambios</button>
              </div>
            )}
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-lg border-t border-slate-100 px-8 py-4 flex justify-between items-center z-40 shadow-2xl">
        {[{id:'library',icon:Layout,l:t.library},{id:'search',icon:Search,l:t.plan},{id:'social',icon:Globe,l:t.social},{id:'profile',icon:User,l:t.profile}].map(t_nav => (
          <button key={t_nav.id} onClick={() => setActiveTab(t_nav.id)} className={`flex flex-col items-center gap-1.5 transition-all ${activeTab === t_nav.id ? 'text-indigo-600 scale-110' : 'text-slate-400 opacity-60'}`}>
