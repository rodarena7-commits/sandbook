...
const i18n = {
  es: {
    ...
    drag_to_scroll: "Arrastra para mover",
    expand: "Expandir",
    collapse: "Colapsar",
    checkpoint_notes_placeholder: "Escribe tus notas del día...",
    page_input_placeholder: "Página actual",
    current_page_progress: "Progreso de página",
    my_notes: "Mis notas",
    reading_plan_status: "Estado del plan",
    days_left: "días restantes",
    users_who_read: "Quienes leyeron",
    users_who_liked: "Quienes dieron like",
    users_who_disliked: "Quienes dieron dislike",
    users_who_favorited: "Quienes favoritaron",
    users_with_book: "Quienes tienen el libro",
    view_likes: "Ver likes",
  ...
  en: {
    ...
    drag_to_scroll: "Drag to scroll",
    expand: "Expand",
    collapse: "Collapse",
    checkpoint_notes_placeholder: "Write your daily notes...",
    page_input_placeholder: "Current page",
    current_page_progress: "Page progress",
    my_notes: "My notes",
    reading_plan_status: "Plan status",
    days_left: "days left",
    users_who_read: "Who read",
    users_who_liked: "Who liked",
    users_who_disliked: "Who disliked",
    users_who_favorited: "Who favorited",
    users_with_book: "Who have the book",
...
const ReadingPlanDetail = ({ book, onToggleCheckpoint, onSaveNote, onSavePage, t, theme, checkpointNotes, currentPageInputs, expandedCheckpoints, onToggleExpand }) => {
        ...
        {sortedCheckpoints.map((cp, idx) => {
          ...
          return (
            ...
            >
              ...
              <button
                onClick={() => onToggleExpand && onToggleExpand(todayKey)}
              ...
              >
                <div className="flex items-center gap-3">
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      onToggleCheckpoint(book.bookId, idx);
                    }}
...
export default function App() {
  ...
  // Estados para scroll de portada
  const [bookCoverScrollY, setBookCoverScrollY] = useState(0);
  const [isDraggingCover, setIsDraggingCover] = useState(false);
  const [coverDragStartY, setCoverDragStartY] = useState(0);

  // Estados para expandir/colapsar checkpoints
  const [expandedCheckpoints, setExpandedCheckpoints] = useState({});

  // Estado para imágenes de escritores
...
const getThemeClasses = () => {
  ...
  // --- FUNCIÓN PARA EXPANDIR/COLAPSAR CHECKPOINTS ---
  const toggleCheckpointExpand = (checkpointKey) => {
    setExpandedCheckpoints(prev => ({
      ...prev,
      [checkpointKey]: prev[checkpointKey] === false ? true : false
    }));
  };

  // --- FUNCIÓN PARA BUSCAR IMAGEN DE ESCRITOR ---
  const fetchWriterImage = async (authorName) => {
    try {
      // Buscar en Wikipedia
      const wikiUrl = `https://es.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(authorName)}`;
      const wikiRes = await fetch(wikiUrl);
      ...
      // Si no se encuentra en Wikipedia, buscar en OpenLibrary
      const olUrl = `https://openlibrary.org/search/authors.json?q=${encodeURIComponent(authorName)}`;
          ...
          if (author.key) {
            ...
            if (authorRes.ok) {
              ...
              if (authorData.photos && authorData.photos.length > 0) {
                return `https://covers.openlibrary.org/a/id/${authorData.photos[0]}-M.jpg`;
              }
            }
          }
        }
      ...
      console.error("Error buscando imagen de escritor:", error);
      return null;
    }
  };
...
const loadBorrowRequests = () => {
  ...
  // --- USEEFFECT PARA CARGAR IMÁGENES DE ESCRITORES ---
  useEffect(() => {
    if (!showFavoriteWriters || favoriteWritersList.length === 0) return;

    const loadWriterImages = async () => {
      const images = {};
      for (const writer of favoriteWritersList) {
        if (!writerImages[writer]) {
          const imageUrl = await fetchWriterImage(writer);
          if (imageUrl) {
            images[writer] = imageUrl;
          }
        }
      }
      if (Object.keys(images).length > 0) {
        setWriterImages(prev => ({ ...prev, ...images }));
      }
    };

    loadWriterImages();
  }, [showFavoriteWriters, favoriteWritersList]);

  useEffect(() => {
    if (!user) return;
    setIsLibraryLoading(true);
    setLibraryLoadingProgress(0);

    // Simular progreso de carga
    const progressInterval = setInterval(() => {
      setLibraryLoadingProgress(prev => {
        if (prev >= 90) {
              ...
              <div className="relative -mt-16 mb-4 flex justify-center">
                <div className="relative group">
                  <img 
                    ...
                    draggable={false}
                    style={{ 
                      transform: `translateY(${bookCoverScrollY}px)`,
                      transition: isDraggingCover ? 'none' : 'transform 0.3s ease-out'
                    }}
                    onMouseDown={(e) => {
                      setIsDraggingCover(true);
                      setCoverDragStartY(e.clientY - bookCoverScrollY);
                    }}
                    onMouseMove={(e) => {
                      if (isDraggingCover) {
                        const newY = e.clientY - coverDragStartY;
                        setBookCoverScrollY(Math.max(-100, Math.min(100, newY)));
                      }
                    }}
                    ...
                    onTouchStart={(e) => {
                      setIsDraggingCover(true);
                      setCoverDragStartY(e.touches[0].clientY - bookCoverScrollY);
                    }}
                    onTouchMove={(e) => {
                      if (isDraggingCover) {
                        const newY = e.touches[0].clientY - coverDragStartY;
                        setBookCoverScrollY(Math.max(-100, Math.min(100, newY)));
                      }
                    }}
                  ...
                  <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="flex flex-col items-center text-white/70 text-[10px]">
                      <ChevronUp size={14} />
                      <span>{t.drag_to_scroll}</span>
                      <ChevronDown size={14} />
                    </div>
                  </div>
                </div>
