...
const getLevelTitle = (count = 0, lang = 'es') => {
  if (count >= 100000) return lang === 'es' ? "Leyenda" : "Legend";
  if (count >= 10000) return lang === 'es' ? "Legendario" : "Legendary";
  if (count >= 1000) return lang === 'es' ? "Profesional" : "Professional";
  if (count >= 100) return lang === 'es' ? "Amateur" : "Amateur";
  if (count >= 10) return lang === 'es' ? "Novato" : "Novice";
  return lang === 'es' ? "Principiante" : "Beginner";
};

// Función para obtener el símbolo del nivel
const getLevelSymbol = (count = 0) => {
  if (count >= 100000) return "₿"; // Bitcoin
  if (count >= 10000) return "🥇"; // Oro
  if (count >= 1000) return "🥈"; // Plata
  if (count >= 100) return "🥉"; // Cobre
  if (count >= 10) return "🔩"; // Acero
  return "🪨"; // Piedra
};
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} pb-24 font-sans overflow-x-hidden selection:bg-indigo-200 selection:text-white`}>
      ...
      {/* TUTORIAL */}
      {showTutorial && (
        <div className="fixed inset-0 z-[999] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          ...
          <div className="space-y-6 animate-in slide-in-from-right-4 pb-12">
            ...
            <div className={`${themeClasses.card} p-8 rounded-[3rem] border ${themeClasses.border} shadow-sm text-center relative overflow-hidden`}>
              ...
              <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase mb-2 tracking-[0.2em]">
                {getLevelTitle(selectedUserProfile.readCount, lang)} <span className="ml-2 text-lg">{getLevelSymbol(selectedUserProfile.readCount)}</span>
              </p>
                  ...
                  <div className="relative z-10 flex justify-between items-start">
                    <div>
                      <div className="flex items-center gap-1.5 mb-1">
                        ...
                        <span className="text-lg">{getLevelSymbol(userProfile.readCount)}</span>
                      </div>
                                ...
                                <div key={p.userId} className={`${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-white'} p-4 rounded-2xl border ${themeClasses.border} flex items-center justify-between`}>
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      <p className="text-[10px] text-slate-500 dark:text-gray-400">
                                        {p.readCount || 0} {t.read.toLowerCase()} • {getLevelTitle(p.readCount, lang)} {getLevelSymbol(p.readCount)}
                                      </p>
                                    </div>
                                  </div>
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6 animate-in fade-in pb-12">
                ...
                <div className={`${themeClasses.card} p-8 rounded-[3rem] border ${themeClasses.border} shadow-sm text-center relative overflow-hidden`}>
                  ...
                  <p className="text-[10px] font-black text-indigo-600 dark:text-indigo-400 uppercase mb-2 tracking-[0.2em]">
                    {getLevelTitle(userProfile.readCount, lang)} <span className="ml-2 text-lg">{getLevelSymbol(userProfile.readCount)}</span>
                  </p>
                ...
                {/* Insignias CON BARRAS DE PORCENTAJE */}
                      ...
                      return (
                        <div key={id} className="flex flex-col items-center gap-1 group relative">
                          <div className="relative w-14 h-14 mb-1">
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all duration-500 transform ${
                              ...
                              {unlocked ? (
                                ...
                                <div className="relative w-full h-full flex items-center justify-center">
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-slate-200 dark:bg-gray-700 rounded-full overflow-hidden">
                                    <div 
                                      className={`h-full ${progress >= 100 ? 'bg-green-500' : 'bg-indigo-500'} transition-all duration-700`}
                            ...
                            {!unlocked && (
                              <div className="absolute -top-1 -right-1 bg-white dark:bg-gray-800 text-[8px] font-bold text-indigo-600 dark:text-indigo-300 rounded-full w-5 h-5 flex items-center justify-center border border-slate-200 dark:border-gray-700">
                                {Math.round(progress)}%
                              </div>
                            )}
                          </div>
