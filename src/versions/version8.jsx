...
import {
  ...
  MoreVertical, UserPlus, BookCopy, Info, Quote, Globe
} from 'lucide-react';
...
const GR = {
  ...
  border:     '#d9d2c5',
  borderLight:'#ede8df',
  shadow:     '0 1px 3px rgba(90,62,43,.10)',
  shadowMd:   '0 4px 12px rgba(90,62,43,.12)',
};
...
const i18n = {
  es: {
    ...
    description:"Descripción", author:"Autor", year:"Año", publisher:"Editorial",
    categories:"Categorías", isbn:"ISBN", pages_total:"Páginas totales",
    book_details:"Detalles del libro", author_books:"Libros del autor",
    see_all:"Ver todos", no_books:"No hay libros",
  },
  en: {
    ...
    description:"Description", author:"Author", year:"Year", publisher:"Publisher",
    categories:"Categories", isbn:"ISBN", pages_total:"Total pages",
    book_details:"Book details", author_books:"Author's books",
    see_all:"See all", no_books:"No books",
  }
};
...
// BookCover async con zoom
const BookCoverAsync = ({bookId, bookData, myBooks, lang, style, onClick, onZoom, showZoomHint=true})=>{
  ...
  return(
    <div style={{position:'relative',display:'inline-block'}}>
      ...
      {showZoomHint && src!=='default' && (onZoom || onClick) && (
        <div style={{position:'absolute',bottom:4,right:4,background:'rgba(0,0,0,.5)',borderRadius:12,padding:'2px 6px',fontSize:8,color:'#fff'}}>
          🔍
        </div>
      )}
      {src==='openlibrary'&&(
...
export default function App() {
  ...
  const [bookDetailActiveTab,setBookDetailActiveTab]=useState('info'); // info, author, similar
  ...
  // ─── FUNCIÓN PARA OBTENER LIBROS DE UN USUARIO ──────────────
  const getUserBooks = async (userId, type = 'all') => {
    try {
      const booksSnap = await getDocs(collection(db, 'users', userId, 'myBooks'));
      let books = booksSnap.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      if (type === 'read') books = books.filter(b => b.status === 'read');
      else if (type === 'reading') books = books.filter(b => b.status === 'reading');
      else if (type === 'liked') books = books.filter(b => userProfile.likes?.includes(b.id));
      return books;
    } catch (error) {
  ...
  // ─── FUNCIÓN PARA OBTENER DETALLES DEL AUTOR ────────────────
  const fetchAuthorDetailsOpenLibrary = async (authorName) => {
    try {
      const res = await fetch(`https://openlibrary.org/search/authors.json?q=${encodeURIComponent(authorName)}`);
      const data = await res.json();
      if (data.docs && data.docs.length > 0) {
        const author = data.docs[0];
        const authorKey = author.key;
        const detailsRes = await fetch(`https://openlibrary.org${authorKey}.json`);
        const details = await detailsRes.json();
        return {
          name: authorName,
          bio: details.bio?.value || `${authorName} es un autor reconocido.`,
          birthDate: details.birth_date,
          deathDate: details.death_date,
          wikipedia: details.wikipedia,
          photos: details.photos || [],
          works: details.works?.length || 0
        };
      }
      return null;
    } catch (error) {
      console.error("Error obteniendo detalles del autor:", error);
      return null;
    }
  };

  // ─── FUNCIÓN PARA BUSCAR LIBROS SIMILARES ───────────────────
  const searchSimilarBooksByGenre = async (title, genres = []) => {
    try {
      let query = '';
      if (genres.length > 0) {
        query = `subject:${encodeURIComponent(genres[0])}`;
      } else {
      ...
      const data = await res.json();
      return data.docs.filter(b => b.title && b.key && b.title !== title).slice(0, 8).map(b => ({
        id: b.key.replace('/works/', ''),
        title: b.title,
        authors: b.author_name || ['Desconocido'],
        coverId: b.cover_i,
  ...
  // ─── FUNCIÓN PARA BUSCAR MÁS DEL MISMO AUTOR ────────────────
  const searchMoreByAuthor = async (authorName, page = 1) => {
    try {
      const res = await fetch(`https://openlibrary.org/search.json?author=${encodeURIComponent(authorName)}&limit=12&page=${page}`);
      const data = await res.json();
      return data.docs.filter(b => b.title && b.key).map(b => ({
        id: b.key.replace('/works/', ''),
        title: b.title,
        authors: [authorName],
        coverId: b.cover_i,
      ...
      console.error("Error buscando más del autor:", error);
      return [];
    }
  };
  ...
  const viewAuthorDetails=async(authorName)=>{
    ...
    try{
      const res=await fetchWithRetry(`https://openlibrary.org/search.json?author=${encodeURIComponent(authorName)}&limit=12`);
      const data=await res.json();
      if(data.docs){
        setAuthorBooks(data.docs.map(b=>({id:b.key?.replace('/works/',''),title:b.title,authors:[authorName],thumbnail:b.cover_i?`https://covers.openlibrary.org/b/id/${b.cover_i}-M.jpg`:null,firstPublishYear:b.first_publish_year})));
      }
      const details=await fetchAuthorDetailsOpenLibrary(authorName);
      setAuthorDetails(details||{name:authorName,bio:`${authorName} es un autor reconocido.`,birthDate:null,deathDate:null});
    }catch(e){console.error(e);}
  };
  ...
  const loadBookRecommendations=async(book)=>{
    ...
    try{
      if(bookAuthors.length>0){
        const authorBooks=await searchMoreByAuthor(bookAuthors[0],1);
        setAuthorRecommendations(authorBooks.filter(b=>b.title!==bookTitle));
      }
      const similar=await searchSimilarBooksByGenre(bookTitle,bookGenres);
      setSimilarBooks(similar.filter(b=>!authorRecommendations.some(a=>a.id===b.id)));
    }catch(e){console.error(e);}
    finally{setLoadingRecommendations(false);}
  };

  const searchMoreBooksByAuthor=async(authorName,page=2)=>{
    setMoreBooksLoading(true);
    try{
      const newBooks=await searchMoreByAuthor(authorName,page);
      setAuthorBooks(p=>[...p,...newBooks.filter(b=>!p.some(e=>e.id===b.id))]);
    }catch(e){console.error(e);}
  ...
  useEffect(()=>{
    ...
    const unsubBooks=onSnapshot(collection(db,'users',user.uid,'myBooks'),(s)=>{
      ...
      setUserStats(calculateUserStats(books));
  ...
  // BookCard mejorada
  const BookCard=({book, isExternal=false, onBorrow, onViewUserBooks})=>(
    <div style={{background:'#fff',border:`1px solid ${GR.border}`,borderRadius:8,padding:14,boxShadow:GR.shadow,display:'flex',gap:12}}>
      <BookCoverAsync bookId={book.bookId||book.id} bookData={book} myBooks={myBooks} lang={lang}
        style={{width:60,height:88,objectFit:'contain',borderRadius:6,background:'#f8f8f8',border:`1px solid ${GR.border}`,flexShrink:0,cursor:'pointer'}}
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        {!isExternal&&(
          <div style={{display:'flex',gap:6,marginTop:6}}>
            <button onClick={()=>openBookDetailModal(book)} style={{...pillInactive,fontSize:10,padding:'3px 10px'}}>{t.view_book_details}</button>
    ...
    <div style={pageStyle}>
      ...
      <div style={{maxWidth:1120,margin:'0 auto',padding:'20px 16px 80px',display:'grid',gridTemplateColumns:'1fr 300px',gap:20,alignItems:'start'}}>
        ...
        <main style={{minWidth:0}}>
          ...
          {/* MURO SOCIAL - TODAS LAS PUBLICACIONES DE TODOS */}
          {activeTab==='social'&&(
            <div>
      ...
      {/* NAV INFERIOR - Mejorado para móvil */}
      ...
      {/* DETALLE DE LIBRO - COMPLETO CON INFORMACIÓN DEL AUTOR Y LIBROS REFERENTES */}
      {viewingBook&&(
        <Modal onClose={()=>{setViewingBook(null);setShowRecommendList(false);}} title={viewingBook.volumeInfo?.title||viewingBook.title} wide>
          ...
          {/* Tabs de información */}
          <div style={{display:'flex',gap:8,borderBottom:`1px solid ${GR.borderLight}`,marginBottom:16}}>
            {[
              ['info',t.book_details],
              ['author',t.author],
              ['similar',t.similar_books]
            ].map(([tab,label])=>(
              <button key={tab} onClick={()=>setBookDetailActiveTab(tab)}
                style={{
                  padding:'8px 16px',background:'none',border:'none',cursor:'pointer',
                  fontFamily:'inherit',fontSize:13,fontWeight:bookDetailActiveTab===tab?700:500,
                  color:bookDetailActiveTab===tab?GR.greenDark:'#888',
                  borderBottom:bookDetailActiveTab===tab?`2px solid ${GR.green}`:'2px solid transparent'
                }}>
                {label}
              </button>
            ))}
          ...
          {/* Contenido según tab */}
          {bookDetailActiveTab==='info'&&(
            <div>
              {/* Descripción */}
              <div style={{marginBottom:16}}>
                <h4 style={{fontSize:13,fontWeight:700,color:GR.brown,margin:'0 0 8px'}}>{t.description}</h4>
                <p style={{fontSize:12,color:'#555',lineHeight:1.7}}>
                  {viewingBook.volumeInfo?.description?.replace(/<[^>]*>/g,'')||viewingBook.description||(lang==='es'?'Sin descripción disponible':'No description available')}
                </p>
              </div>
              
              {/* Información adicional */}
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,marginBottom:16}}>
                {viewingBook.volumeInfo?.pageCount&&(
                  <div><span style={{fontSize:11,color:'#aaa'}}>{t.pages_total}:</span> <span style={{fontSize:12,fontWeight:600}}>{viewingBook.volumeInfo.pageCount}</span></div>
                )}
                {viewingBook.volumeInfo?.publishedDate&&(
                  <div><span style={{fontSize:11,color:'#aaa'}}>{t.year}:</span> <span style={{fontSize:12,fontWeight:600}}>{viewingBook.volumeInfo.publishedDate}</span></div>
                )}
                {viewingBook.volumeInfo?.publisher&&(
                  <div><span style={{fontSize:11,color:'#aaa'}}>{t.publisher}:</span> <span style={{fontSize:12,fontWeight:600}}>{viewingBook.volumeInfo.publisher}</span></div>
                )}
                {viewingBook.volumeInfo?.categories?.length>0&&(
                  <div><span style={{fontSize:11,color:'#aaa'}}>{t.categories}:</span> <span style={{fontSize:12,fontWeight:600}}>{viewingBook.volumeInfo.categories.slice(0,2).join(', ')}</span></div>
                )}
                {viewingBook.volumeInfo?.industryIdentifiers?.find(id=>id.type==='ISBN_13')?.identifier&&(
                  <div><span style={{fontSize:11,color:'#aaa'}}>{t.isbn}:</span> <span style={{fontSize:11}}>{viewingBook.volumeInfo.industryIdentifiers.find(id=>id.type==='ISBN_13').identifier}</span></div>
                )}
              </div>
          ...
          {bookDetailActiveTab==='author'&&(
            <div>
              <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:16}}>
                <div style={{width:56,height:56,borderRadius:'50%',background:GR.creamDark,display:'flex',alignItems:'center',justifyContent:'center'}}>
                  <PenTool size={28} color={GR.brownMid}/>
                </div>
                <div>
                  <h4 style={{fontSize:15,fontWeight:700,color:GR.brown,margin:'0 0 4px'}}>
                    {viewingBook.volumeInfo?.authors?.join(', ')||viewingBook.authors?.join(', ')}
                  </h4>
                  <button onClick={()=>{
                    const authorName=(viewingBook.volumeInfo?.authors||viewingBook.authors)?.[0];
                    if(authorName){setViewingBook(null);viewAuthorDetails(authorName);}
                  }} style={{fontSize:11,color:GR.greenDark,background:'none',border:'none',cursor:'pointer',textDecoration:'underline'}}>
                    {t.view_profile}
                  </button>
                </div>
              </div>
              <div style={{marginBottom:16}}>
                <h4 style={{fontSize:12,fontWeight:700,color:GR.brown,margin:'0 0 6px'}}>{t.books_by_author}</h4>
                {loadingRecommendations?(
                  <div style={{textAlign:'center',padding:16}}><Loader2 size={24} style={{animation:'spin 1s linear infinite'}}/></div>
                ):authorRecommendations.length===0?(
                  <p style={{fontSize:11,color:'#aaa',textAlign:'center',padding:16}}>{lang==='es'?'No se encontraron más libros del autor':'No more books by this author'}</p>
                ):(
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8}}>
                    {authorRecommendations.slice(0,6).map(book=>(
                      <div key={book.id} onClick={()=>{setViewingBook({id:book.id,volumeInfo:{title:book.title,authors:[viewingBook.volumeInfo?.authors?.[0]],imageLinks:{thumbnail:book.thumbnail}}});}}
                        style={{cursor:'pointer',textAlign:'center'}}>
                        <img src={book.thumbnail||'https://via.placeholder.com/80x120'} alt=""
          ...
          {bookDetailActiveTab==='similar'&&(
            <div>
              <h4 style={{fontSize:13,fontWeight:700,color:GR.brown,margin:'0 0 12px'}}>{t.recommended_books}</h4>
              {loadingRecommendations?(
                <div style={{textAlign:'center',padding:16}}><Loader2 size={24} style={{animation:'spin 1s linear infinite'}}/></div>
              ):similarBooks.length===0?(
                <p style={{fontSize:11,color:'#aaa',textAlign:'center',padding:16}}>{lang==='es'?'No se encontraron libros similares':'No similar books found'}</p>
              ):(
                <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>
                  {similarBooks.slice(0,6).map(book=>(
                    <div key={book.id} onClick={()=>{setViewingBook({id:book.id,volumeInfo:{title:book.title,authors:book.authors,imageLinks:{thumbnail:book.thumbnail}}});}}
                      style={{cursor:'pointer',textAlign:'center'}}>
                      <img src={book.thumbnail||'https://via.placeholder.com/80x120'} alt=""
                        style={{width:'100%',aspectRatio:'2/3',objectFit:'contain',borderRadius:4,background:'#f8f8f8',border:`1px solid ${GR.border}`}}/>
                      <p style={{fontSize:10,fontWeight:600,color:GR.brown,margin:'4px 0 0',lineHeight:1.3}}>{book.title}</p>
                      <p style={{fontSize:9,color:'#aaa',margin:0}}>{book.authors?.[0]}</p>
                    </div>
                  ))}
                </div>
              )}
          ...
          {/* Recomendar */}
          {showRecommendList&&(
            <div style={{background:GR.parchment,border:`1px solid ${GR.borderLight}`,borderRadius:8,padding:14,marginTop:14}}>
            ...
            <div>
              ...
              <div style={{display:'flex',gap:14,marginBottom:14}}>
                ...
                <div style={{flex:1}}>
                  ...
                  {authorDetails?.bio&&<p style={{fontSize:12,color:'#888',margin:'0 0 8px',lineHeight:1.5}}>{authorDetails.bio}</p>}
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                {authorBooks.map((book,i)=>(
                  <div key={i} onClick={()=>{setShowWriters(false);setViewingBook(book);}}
                    style={{cursor:'pointer',textAlign:'center'}}>
                    <img src={book.thumbnail||'https://via.placeholder.com/100x150?text=NO+COVER'} alt=""
                      style={{width:'100%',aspectRatio:'2/3',objectFit:'contain',borderRadius:4,background:'#f8f8f8',border:`1px solid ${GR.border}`}}/>
                    <p style={{fontSize:10,fontWeight:600,color:GR.brown,margin:'4px 0 0',lineHeight:1.3}}>{book.title}</p>
                  </div>
                ))}
              ...
              {authorBooks.length>=10&&(
                <button onClick={()=>searchMoreBooksByAuthor(selectedAuthor)} disabled={moreBooksLoading}
                  ...
                  {moreBooksLoading?<Loader2 size={14} style={{animation:'spin 1s linear infinite'}}/>:`${t.load_more}`}
                </button>
              )}
            </div>
          )}
      ...
      {/* PERFIL EXTERNO - CON LIBROS DEL USUARIO */}
      {selectedUserProfile&&(
        <Modal onClose={()=>setSelectedUserProfile(null)} title={selectedUserProfile.name} titleBg={GR.brownMid} wide>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,maxHeight:400,overflowY:'auto'}}>
            {filteredExternalBooks.length===0?<p style={{gridColumn:'1/-1',textAlign:'center',color:'#aaa',fontSize:12,padding:24}}>{t.no_books}</p>:
