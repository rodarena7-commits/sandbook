...
// Modal base — bottom sheet en mobile, centrado en desktop
const Modal = ({onClose, title, titleBg=GR.brown, children, wide})=>{
  ...
  <div style={{
    ...
    <div className="sb-modal-inner" style={{
      background:'#fff', width:'100%', maxWidth:wide?640:480,
      maxHeight:mobile?'95vh':'90vh', borderRadius:mobile?'20px 20px 0 0':16,
      overflow:'hidden', display:'flex', flexDirection:'column',
      boxShadow:'0 -8px 40px rgba(0,0,0,.25)'
    }}>
      {title&&(
        <div style={{
          background:titleBg, padding:mobile?'14px 16px':'14px 20px',
          display:'flex', alignItems:'center', justifyContent:'space-between',
          flexShrink:0
        }}>
          <h2 style={{
            fontFamily:"'Playfair Display', Georgia, serif",
            fontSize:mobile?15:18, fontWeight:700, color:'#fff', margin:0
          }}>{title}</h2>
      ...
      <div style={{flex:1, overflowY:'auto', padding:mobile?16:20}}>{children}</div>
    </div>
  </div>
);};
...
export default function App() {
  ...
  // ── NUEVOS ESTADOS ──
  const [viewingUserBooks,setViewingUserBooks]=useState(null); // {user, tab:'read'|'reading'|'liked'}
  const [viewingUserBooksData,setViewingUserBooksData]=useState([]);
  const [bookDetailTab,setBookDetailTab]=useState('info'); // 'info'|'author'|'refs'|'reviews'
  const [bookAuthorData,setBookAuthorData]=useState(null);
  ...
  const loadWallPosts=()=>{
    ...
    return onSnapshot(collection(db,'wallPosts'),(snap)=>{
      // Show ALL public posts from all users
      const posts=snap.docs.map(d=>({id:d.id,...d.data()})).filter(p=>p.isPublic!==false);
      posts.sort((a,b)=>(b.timestamp?.seconds||0)-(a.timestamp?.seconds||0));
      setWallPosts(posts.slice(0,100));
    });
  };
  ...
  const openBookDetailModal=(bookData)=>{
    ...
    setBookDetailTab('info');
    setBookAuthorData(null);
    setBookRefs([]);
    loadBookRecommendations(bookData);
  };
  const openBadgeModal=(badgeId)=>{setSelectedBadge({id:badgeId,...BADGE_DEFS[badgeId]});setShowBadgeModal(true);};

  const loadBookAuthorAndRefs=async(book)=>{
    const title=book.volumeInfo?.title||book.title;
    const authors=book.volumeInfo?.authors||book.authors||[];
    if(!authors[0]) return;
    setLoadingBookDetail(true);
    try{
      // Author data from Wikipedia
      const wikiRes=await fetch(`https://es.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(authors[0])}`);
      if(wikiRes.ok){
        const wikiData=await wikiRes.json();
        setBookAuthorData({name:authors[0],biography:wikiData.extract||'',thumbnail:wikiData.thumbnail?.source||'',description:wikiData.description||''});
      }else{
        setBookAuthorData({name:authors[0],biography:'',thumbnail:'',description:'Autor'});
      }
      // Related books
      const keywords=title.split(' ').filter(w=>w.length>3).slice(0,3).join(' ');
      const relRes=await fetchWithRetry(`https://openlibrary.org/search.json?q=${encodeURIComponent(keywords)}&limit=12`);
      const relData=await relRes.json();
      const refs=(relData.docs||[]).filter(b=>b.title&&b.key&&b.title!==title).slice(0,9).map(b=>({
        id:b.key.replace('/works/',''),title:b.title,
  ...
  const openUserBooksModal=(userData,tab='read')=>{
    setViewingUserBooks({user:userData,tab});
    setViewingUserBooksData([]);
    // load books for that user
    const unsub=onSnapshot(collection(db,'users',userData.userId,'myBooks'),(s)=>{
      setViewingUserBooksData(s.docs.map(d=>d.data()));
    });
    // store unsub to clean up on close
    viewingUserBooksUnsub.current=unsub;
  };
  const viewingUserBooksUnsub=useRef(null);
    if(librarySearch){
  ...
  return(
    <div style={pageStyle}>
        ...
        <div style={{maxWidth:1120,margin:'0 auto',display:'flex',alignItems:'center',gap:12,padding:'0 16px',height:52}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:isMobile?4:6,marginLeft:'auto',flexShrink:0}}>
            ...
            <div ref={notificationsRef} style={{position:'relative'}}>
              ...
              {showNotifications&&(
                <div style={{position:'absolute',right:0,top:40,width:isMobile?'calc(100vw - 20px)':300,maxHeight:400,overflowY:'auto',background:'#fff',borderRadius:8,boxShadow:'0 8px 24px rgba(0,0,0,.2)',border:`1px solid ${GR.border}`,zIndex:200}}>
                  <div style={{padding:'10px 14px',borderBottom:`1px solid ${GR.borderLight}`,fontWeight:700,fontSize:13,color:GR.brown}}>{lang==='es'?'Notificaciones':'Notifications'}</div>
                  {notifications.length===0?<p style={{padding:16,fontSize:12,color:'#999',textAlign:'center'}}>{lang==='es'?'Sin notificaciones':'No notifications'}</p>:
                    notifications.map(n=>(
            ...
            {!isMobile&&<button onClick={()=>setLang(lang==='es'?'en':'es')}
            ...
            {/* Avatar */}
            <div style={{width:32,height:32,borderRadius:'50%',overflow:'hidden',border:'2px solid rgba(255,255,255,.35)',flexShrink:0,cursor:'pointer'}} onClick={()=>setActiveTab('profile')}>
      ...
      {/* MAIN CONTENT */}
      <div style={{maxWidth:1120,margin:'0 auto',padding:isMobile?'12px 10px 80px':'20px 16px 80px',display:isMobile?'block':'grid',gridTemplateColumns:'1fr 300px',gap:20,alignItems:'start'}}>
        ...
        <main style={{minWidth:0}}>
          ...
          {activeTab==='profile'&&(
            <div>
              ...
              {/* Stats grid */}
              <div className="sb-stats-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10,marginBottom:16}}>
                {[
        ...
        {/* SIDEBAR GOODREADS - hidden on mobile */}
        {!isMobile&&<aside style={{minWidth:0}}>
      ...
      {/* NAV INFERIOR */}
      <nav className="sb-bottom-nav" style={{position:'fixed',bottom:0,left:0,right:0,background:'#fff',borderTop:`1px solid ${GR.border}`,display:'flex',boxShadow:'0 -2px 12px rgba(90,62,43,.08)',zIndex:100}}>
        {[
      ...
      {/* DETALLE DE LIBRO - con pestañas: Info, Autor, Referencias, Reseñas */}
      {viewingBook&&(
        <Modal onClose={()=>{setViewingBook(null);setShowRecommendList(false);setBookDetailTab('info');setBookAuthorData(null);setBookRefs([]);}} title="" wide>
          {/* Hero del libro */}
          <div style={{display:'flex',gap:16,marginBottom:0,background:`linear-gradient(135deg, ${GR.brown} 0%, ${GR.brownMid} 100%)`,margin:'-20px -20px 0',padding:'20px 20px 0',borderRadius:'0'}}>
            <div style={{flexShrink:0}}>
              <BookCoverAsync bookId={viewingBook.id||viewingBook.bookId} bookData={viewingBook} myBooks={myBooks} lang={lang}
                style={{width:isMobile?80:100,height:isMobile?120:148,objectFit:'contain',borderRadius:8,background:'#f8f8f8',border:'2px solid rgba(255,255,255,.3)',cursor:'pointer',boxShadow:'0 8px 24px rgba(0,0,0,.3)'}}
                onZoom={(url,ttl)=>{setZoomImage(url);setZoomTitle(ttl);}}/>
            </div>
            <div style={{flex:1,color:'#fff',minWidth:0}}>
              <h2 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:isMobile?16:20,fontWeight:700,color:'#fff',margin:'0 0 4px',lineHeight:1.3}}>{viewingBook.volumeInfo?.title||viewingBook.title}</h2>
              <p onClick={()=>{const a=(viewingBook.volumeInfo?.authors||viewingBook.authors)?.[0];if(a){setViewingBook(null);viewAuthorDetails(a);}}}
                style={{fontSize:13,color:'rgba(255,255,255,.8)',margin:'0 0 8px',cursor:'pointer',fontStyle:'italic',textDecoration:'underline'}}>
                {viewingBook.volumeInfo?.authors?.join(', ')||viewingBook.authors?.join(', ')}
              </p>
              {viewingBook.volumeInfo?.pageCount&&<p style={{fontSize:11,color:'rgba(255,255,255,.6)',margin:'0 0 8px'}}>{viewingBook.volumeInfo.pageCount} {t.pages} · {viewingBook.volumeInfo?.publishedDate||''}</p>}
              <div style={{display:'flex',gap:6,flexWrap:'wrap',marginBottom:8}}>
                <BtnGR small onClick={()=>handleAddBook(viewingBook,'library',false,true)} disabled={!!myBooks.find(b=>b.bookId===(viewingBook.id||viewingBook.bookId))} color={GR.green} darkColor={GR.greenDark}>
                  {myBooks.find(b=>b.bookId===(viewingBook.id||viewingBook.bookId))?'✓ '+t.in_your_library:t.add_to_library}
                </BtnGR>
                <BtnGR small onClick={()=>{setPlanningBook(viewingBook);setViewingBook(null);}} color={GR.brownLight} darkColor={GR.brownMid}>{t.reading_plan}</BtnGR>
              ...
              <div style={{display:'flex',gap:6,alignItems:'center'}}>
                <button onClick={()=>handleGlobalBookReaction(viewingBook.id||viewingBook.bookId,'like')}
                  style={{display:'flex',alignItems:'center',gap:4,padding:'4px 10px',borderRadius:20,border:'1px solid rgba(255,255,255,.4)',background:globalLikes[viewingBook.id||viewingBook.bookId]?.likes?.includes(user?.uid)?'rgba(255,255,255,.25)':'transparent',cursor:'pointer',fontSize:12,color:'#fff'}}>
                  <ThumbsUp size={12}/> {globalLikes[viewingBook.id||viewingBook.bookId]?.likes?.length||0}
                </button>
                <button onClick={()=>handleGlobalBookReaction(viewingBook.id||viewingBook.bookId,'dislike')}
                  style={{display:'flex',alignItems:'center',gap:4,padding:'4px 10px',borderRadius:20,border:'1px solid rgba(255,255,255,.4)',background:globalLikes[viewingBook.id||viewingBook.bookId]?.dislikes?.includes(user?.uid)?'rgba(255,255,255,.25)':'transparent',cursor:'pointer',fontSize:12,color:'#fff'}}>
                  <ThumbsDown size={12}/> {globalLikes[viewingBook.id||viewingBook.bookId]?.dislikes?.length||0}
                </button>
                <span style={{fontSize:11,color:'rgba(255,255,255,.5)',marginLeft:4}}>{getReadersCount(viewingBook.id||viewingBook.bookId)} {lang==='es'?'lectores':'readers'}</span>
              </div>
            </div>
          </div>
          {/* Tabs */}
          <div style={{display:'flex',borderBottom:`1px solid ${GR.borderLight}`,margin:'0 -20px',padding:'0 4px',background:'#fff',overflowX:'auto'}}>
            {[
              ['info',lang==='es'?'📖 Info':'📖 Info'],
              ['author',lang==='es'?'✍️ Autor':'✍️ Author'],
              ['refs',lang==='es'?'🔗 Relacionados':'🔗 Related'],
              ['reviews',lang==='es'?'⭐ Reseñas':'⭐ Reviews'],
            ].map(([tab,label])=>(
              <button key={tab}
                onClick={()=>{
                  setBookDetailTab(tab);
                  if((tab==='author'||tab==='refs')&&!bookAuthorData&&!loadingBookDetail) loadBookAuthorAndRefs(viewingBook);
                }}
                style={{padding:'10px 14px',background:'none',border:'none',borderBottom:bookDetailTab===tab?`3px solid ${GR.green}`:'3px solid transparent',cursor:'pointer',fontSize:12,fontWeight:bookDetailTab===tab?700:500,color:bookDetailTab===tab?GR.green:GR.brown,whiteSpace:'nowrap',fontFamily:'inherit',flexShrink:0}}>
                {label}
              </button>
            ))}
          </div>
          {/* Tab: Info (descripción + contratapa + plan activo) */}
          {bookDetailTab==='info'&&(
            <div style={{paddingTop:16}}>
              ...
              {(viewingBook.volumeInfo?.description||viewingBook.description)&&(
                <div style={{marginBottom:16}}>
                  <h4 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:13,fontWeight:700,color:GR.brown,margin:'0 0 8px'}}>
                    {lang==='es'?'Sinopsis':'Synopsis'}
                  </h4>
                  <p style={{fontSize:13,color:'#555',lineHeight:1.75,margin:0}}>
                    {(viewingBook.volumeInfo?.description||viewingBook.description||'').replace(/<[^>]*>/g,'')}
                  </p>
                </div>
              )}
              {/* Info adicional */}
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8,marginBottom:16}}>
                {[
                  [lang==='es'?'Páginas':'Pages', viewingBook.volumeInfo?.pageCount||'—'],
                  [lang==='es'?'Publicado':'Published', viewingBook.volumeInfo?.publishedDate||'—'],
                  [lang==='es'?'Editorial':'Publisher', viewingBook.volumeInfo?.publisher||'—'],
                  [lang==='es'?'Idioma':'Language', viewingBook.volumeInfo?.language?.toUpperCase()||'—'],
                ].filter(([,v])=>v&&v!=='—').map(([label,val],i)=>(
                  <div key={i} style={{background:GR.parchment,borderRadius:8,padding:'10px 12px',border:`1px solid ${GR.borderLight}`}}>
                    <div style={{fontSize:9,color:'#aaa',textTransform:'uppercase',letterSpacing:.5,marginBottom:2}}>{label}</div>
                    <div style={{fontSize:13,fontWeight:700,color:GR.brown}}>{val}</div>
                  </div>
                ))}
              </div>
              {/* Categorías */}
              {viewingBook.volumeInfo?.categories?.length>0&&(
                <div style={{marginBottom:16,display:'flex',gap:6,flexWrap:'wrap'}}>
                  {viewingBook.volumeInfo.categories.map((cat,i)=>(
                    <span key={i} style={{padding:'3px 10px',background:GR.creamDark,borderRadius:20,fontSize:11,color:GR.brownMid,fontWeight:600}}>{cat}</span>
                  ))}
                </div>
              )}
              {/* Plan activo */}
          ...
          {/* Tab: Autor */}
          {bookDetailTab==='author'&&(
            <div style={{paddingTop:16}}>
              {loadingBookDetail&&!bookAuthorData?(
                <div style={{textAlign:'center',padding:32}}><Loader2 size={28} color={GR.green} style={{animation:'spin 1s linear infinite',margin:'0 auto'}}/></div>
              ):bookAuthorData?(
                <div>
                  <div style={{display:'flex',gap:14,marginBottom:16}}>
                    {bookAuthorData.thumbnail?(
                      <img src={bookAuthorData.thumbnail} alt="" style={{width:72,height:72,borderRadius:'50%',objectFit:'cover',border:`2px solid ${GR.border}`,flexShrink:0}}/>
                    ):(
                      <div style={{width:72,height:72,borderRadius:'50%',background:GR.creamDark,display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                        <PenTool size={28} color={GR.brownMid}/>
                      </div>
                    )}
                    <div>
                      <h3 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:18,fontWeight:700,color:GR.brown,margin:'0 0 4px'}}>{bookAuthorData.name}</h3>
                      {bookAuthorData.description&&<p style={{fontSize:12,color:'#888',margin:'0 0 8px'}}>{bookAuthorData.description}</p>}
                      <BtnGR small onClick={()=>{setViewingBook(null);viewAuthorDetails(bookAuthorData.name);setShowWriters(true);}} color={GR.brownMid} darkColor={GR.brown}>
                        {lang==='es'?'Ver todos sus libros':'See all books'}
                      </BtnGR>
                    </div>
                  </div>
                  {bookAuthorData.biography&&(
                    <div style={{background:GR.parchment,border:`1px solid ${GR.borderLight}`,borderRadius:8,padding:14}}>
                      ...
                      <p style={{fontSize:13,color:'#555',lineHeight:1.75,margin:0}}>{bookAuthorData.biography}</p>
                    </div>
                  )}
                </div>
              ):(
                <p style={{textAlign:'center',color:'#aaa',fontSize:12,padding:24}}>{lang==='es'?'No hay información del autor':'No author info available'}</p>
              )}
            </div>
          )}
          {/* Tab: Relacionados */}
          {bookDetailTab==='refs'&&(
            <div style={{paddingTop:16}}>
              {loadingBookDetail&&bookRefs.length===0?(
                <div style={{textAlign:'center',padding:32}}><Loader2 size={28} color={GR.green} style={{animation:'spin 1s linear infinite',margin:'0 auto'}}/></div>
              ):bookRefs.length>0?(
                <div>
                  <p style={{fontSize:11,color:'#aaa',marginBottom:12}}>{lang==='es'?'Libros relacionados que podrían interesarte:':'Related books you might enjoy:'}</p>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                    {bookRefs.map((ref,i)=>(
                      <div key={i} onClick={()=>{setViewingBook({id:ref.id,volumeInfo:{title:ref.title,authors:ref.authors},bookId:ref.id});setBookDetailTab('info');setBookAuthorData(null);setBookRefs([]);}}
                        style={{cursor:'pointer',textAlign:'center'}}>
                        {ref.thumbnail?(
                          <img src={ref.thumbnail} alt="" style={{width:'100%',aspectRatio:'2/3',objectFit:'contain',borderRadius:6,background:'#f8f8f8',border:`1px solid ${GR.border}`}}/>
                        ):(
                          <div style={{width:'100%',aspectRatio:'2/3',background:'#f0ede6',borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',border:`1px solid ${GR.border}`}}>
                            <BookOpen size={24} color={GR.border}/>
                          </div>
                        )}
                        <p style={{fontSize:10,fontWeight:600,color:GR.brown,margin:'4px 0 2px',lineHeight:1.3,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{ref.title}</p>
                        <p style={{fontSize:9,color:'#aaa',margin:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{ref.authors?.[0]||''}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ):(
                <p style={{textAlign:'center',color:'#aaa',fontSize:12,padding:24}}>{lang==='es'?'No se encontraron libros relacionados':'No related books found'}</p>
              )}
            </div>
          )}
          {/* Tab: Reseñas */}
          {bookDetailTab==='reviews'&&(
            <div style={{paddingTop:16}}>
              <div style={{background:GR.parchment,border:`1px solid ${GR.borderLight}`,borderRadius:8,padding:14,marginBottom:16}}>
                <p style={{fontSize:11,fontWeight:700,color:'#888',textTransform:'uppercase',letterSpacing:.5,margin:'0 0 8px'}}>{t.my_review}</p>
                <Stars rating={userRating} interactive onRate={setUserRating} size={22}/>
              ...
              {(bookComments[viewingBook.id||viewingBook.bookId]||[]).length===0?(
                <p style={{textAlign:'center',color:'#aaa',fontSize:12,padding:24}}>{lang==='es'?'Sin reseñas aún':'No reviews yet'}</p>
              ):(
                (bookComments[viewingBook.id||viewingBook.bookId]||[]).map(c=>(
                  <div key={c.id} style={{background:GR.parchment,borderRadius:6,padding:'10px 12px',marginBottom:8}}>
                    <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                      <img src={c.userPic||'https://via.placeholder.com/24'} alt="" style={{width:28,height:28,borderRadius:'50%',objectFit:'cover'}}/>
                      <div>
                        <span style={{fontSize:12,fontWeight:700,color:GR.brown,display:'block'}}>{c.userName}</span>
                        <Stars rating={c.rating} size={11}/>
                      </div>
                    </div>
                    <p style={{fontSize:12,color:'#555',margin:0,lineHeight:1.6}}>{c.text}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {showUserProfileModal&&userProfileModalData&&(
        <Modal onClose={()=>setShowUserProfileModal(false)} title="" titleBg={GR.brown} wide>
          {/* Header */}
          <div style={{margin:'-20px -20px 0',background:`linear-gradient(135deg, ${GR.brown} 0%, ${GR.brownMid} 100%)`,padding:'20px 20px 16px',display:'flex',gap:14,alignItems:'center'}}>
            <img src={userProfileModalData.profilePic||'https://via.placeholder.com/64'} alt=""
              style={{width:64,height:64,borderRadius:'50%',objectFit:'cover',border:'3px solid rgba(255,255,255,.4)',flexShrink:0}}/>
            <div style={{flex:1,color:'#fff'}}>
              <h3 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:18,fontWeight:700,color:'#fff',margin:'0 0 2px'}}>{userProfileModalData.name}</h3>
              <p style={{fontSize:11,color:'rgba(255,255,255,.6)',margin:'0 0 8px'}}>{getLevelTitle(userProfileModalData.readCount,lang)} <VerificationCheck count={userProfileModalData.readCount}/></p>
              <div style={{display:'flex',gap:6}}>
                <BtnGR small onClick={()=>{toggleFollow(userProfileModalData.userId);setShowUserProfileModal(false);}} color={userProfile.following?.includes(userProfileModalData.userId)?'rgba(255,255,255,.2)':GR.green}>
                  {userProfile.following?.includes(userProfileModalData.userId)?t.unfollow:t.add_friend}
                </BtnGR>
                <BtnGR small onClick={()=>{setSelectedUserForMessage(userProfileModalData);setShowUserProfileModal(false);setShowNewMessageModal(true);}} color="rgba(255,255,255,.2)">
                  <MessageSquare size={13}/> {t.send_message}
                </BtnGR>
              </div>
            </div>
          ...
          {/* Stats */}
          <div style={{display:'flex',borderBottom:`1px solid ${GR.borderLight}`,marginBottom:14}}>
            {[
              [userProfileModalData.readCount||0,lang==='es'?'Leídos':'Read','read'],
              [userProfileModalData.planCount||0,lang==='es'?'Leyendo':'Reading','reading'],
              [userProfileModalData.likes?.length||0,lang==='es'?'Les gusta':'Liked','liked'],
            ].map(([num,label,tab],i)=>(
              <button key={i} onClick={()=>openUserBooksModal(userProfileModalData,tab)}
                style={{flex:1,textAlign:'center',padding:'10px 4px',background:'none',border:'none',cursor:'pointer',borderBottom:2===i?'':'none'}}>
                <div style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:20,fontWeight:700,color:GR.brown}}>{num}</div>
                <div style={{fontSize:9,color:'#aaa',textTransform:'uppercase',letterSpacing:.3}}>{label}</div>
              </button>
            ))}
          </div>
          {/* Recent books preview */}
          <div style={{marginBottom:12}}>
            <p style={{fontSize:11,fontWeight:700,color:'#aaa',textTransform:'uppercase',letterSpacing:.5,marginBottom:8}}>{lang==='es'?'Últimos libros leídos':'Recently read'}</p>
            <div style={{display:'flex',gap:8,overflowX:'auto',paddingBottom:4}}>
              {(selectedUserBooks.length>0?selectedUserBooks:viewingUserBooksData).filter(b=>b.status==='read').slice(0,8).map((book,i)=>(
                <div key={i} onClick={()=>{setShowUserProfileModal(false);setViewingBook({id:book.bookId,bookId:book.bookId,title:book.title,authors:book.authors,thumbnail:book.thumbnail,volumeInfo:{title:book.title,authors:book.authors}});}}
                  style={{flexShrink:0,cursor:'pointer'}}>
                  <img src={book.thumbnail||'https://via.placeholder.com/50x75'} alt=""
                    style={{width:50,height:75,objectFit:'contain',borderRadius:4,background:'#f8f8f8',border:`1px solid ${GR.border}`}}/>
                </div>
              ))}
            </div>
          </div>
          <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
            <BtnGR small onClick={()=>{setShowUserProfileModal(false);openUserBooksModal(userProfileModalData,'read');}} color={GR.green}>
              <BookOpen size={13}/> {lang==='es'?'Ver libros leídos':'Read books'}
            </BtnGR>
            <BtnGR small onClick={()=>{setShowUserProfileModal(false);openUserBooksModal(userProfileModalData,'reading');}} color={GR.brownMid} darkColor={GR.brown}>
              <BookOpen size={13}/> {lang==='es'?'Leyendo ahora':'Now reading'}
            </BtnGR>
            <BtnGR small onClick={()=>{setShowUserProfileModal(false);openUserBooksModal(userProfileModalData,'liked');}} color={GR.gold} darkColor="#a07020">
              <ThumbsUp size={13}/> {lang==='es'?'Les gusta':'Liked'}
            </BtnGR>
          </div>
        </Modal>
      )}
      ...
      {/* LIBROS DE USUARIO - por categoría */}
      {viewingUserBooks&&(
        <div style={{position:'fixed',inset:0,zIndex:500,background:'rgba(0,0,0,.75)',display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'#fff',width:'100%',maxWidth:600,borderRadius:'20px 20px 0 0',maxHeight:'90vh',display:'flex',flexDirection:'column',boxShadow:'0 -8px 40px rgba(0,0,0,.25)'}}>
            {/* Header */}
            <div style={{background:`linear-gradient(135deg, ${GR.brown} 0%, ${GR.brownMid} 100%)`,padding:'16px 20px',borderRadius:'20px 20px 0 0',display:'flex',alignItems:'center',gap:12,flexShrink:0}}>
              <img src={viewingUserBooks.user.profilePic||'https://via.placeholder.com/40'} alt=""
                style={{width:44,height:44,borderRadius:'50%',objectFit:'cover',border:'2px solid rgba(255,255,255,.4)',flexShrink:0}}/>
              <div style={{flex:1}}>
                <h3 style={{fontFamily:"'Playfair Display',Georgia,serif",fontSize:16,fontWeight:700,color:'#fff',margin:0}}>{viewingUserBooks.user.name}</h3>
                <p style={{fontSize:11,color:'rgba(255,255,255,.6)',margin:0}}>{getLevelTitle(viewingUserBooks.user.readCount,lang)}</p>
              </div>
              <button onClick={()=>{setViewingUserBooks(null);if(viewingUserBooksUnsub.current){viewingUserBooksUnsub.current();viewingUserBooksUnsub.current=null;}}}
                style={{width:32,height:32,borderRadius:'50%',background:'rgba(255,255,255,.2)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff'}}>
                <X size={16}/>
              </button>
            </div>
            {/* Tabs de categoría */}
            <div style={{display:'flex',borderBottom:`1px solid ${GR.borderLight}`,flexShrink:0,background:'#fff'}}>
              {[
                ['read',lang==='es'?'📗 Leídos':'📗 Read'],
                ['reading',lang==='es'?'📖 Leyendo':'📖 Reading'],
                ['liked',lang==='es'?'👍 Les gusta':'👍 Liked'],
              ].map(([tab,label])=>(
                <button key={tab}
                  onClick={()=>setViewingUserBooks(v=>({...v,tab}))}
                  style={{flex:1,padding:'10px 6px',background:'none',border:'none',borderBottom:viewingUserBooks.tab===tab?`3px solid ${GR.green}`:'3px solid transparent',cursor:'pointer',fontSize:12,fontWeight:viewingUserBooks.tab===tab?700:500,color:viewingUserBooks.tab===tab?GR.green:GR.brown,fontFamily:'inherit',whiteSpace:'nowrap'}}>
                  {label}
                </button>
              ))}
            </div>
            ...
            <div style={{flex:1,overflowY:'auto',padding:16}}>
              {(()=>{
                const books=viewingUserBooksData.filter(b=>{
                  if(viewingUserBooks.tab==='read') return b.status==='read';
                  if(viewingUserBooks.tab==='reading') return b.status==='reading';
                  if(viewingUserBooks.tab==='liked') return viewingUserBooks.user.likes?.includes(b.bookId);
                  return false;
                });
                if(books.length===0) return(
                  <div style={{textAlign:'center',padding:'48px 16px'}}>
                    <BookOpen size={40} color={GR.border} style={{margin:'0 auto 10px'}}/>
                    <p style={{color:'#aaa',fontSize:12}}>{lang==='es'?'No hay libros en esta categoría':'No books in this category'}</p>
                  </div>
                );
                return(
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:10}}>
                    {books.map((book,i)=>(
                      <div key={i} onClick={()=>{
                        setViewingUserBooks(null);
                        if(viewingUserBooksUnsub.current){viewingUserBooksUnsub.current();viewingUserBooksUnsub.current=null;}
                        setViewingBook({id:book.bookId,bookId:book.bookId,title:book.title,authors:book.authors,thumbnail:book.thumbnail,volumeInfo:{title:book.title,authors:book.authors,description:book.description,pageCount:book.totalPages}});
                      }} style={{cursor:'pointer'}}>
                        <div style={{position:'relative',marginBottom:6}}>
                          <img src={book.thumbnail||'https://via.placeholder.com/100x150?text=?'} alt=""
                            style={{width:'100%',aspectRatio:'2/3',objectFit:'contain',borderRadius:6,background:'#f8f8f8',border:`1px solid ${GR.border}`,display:'block'}}
                            onError={e=>{e.target.src='https://via.placeholder.com/100x150?text=?';}}/>
                          {book.status==='reading'&&book.checkpoints?.length>0&&(
                            <div style={{position:'absolute',bottom:0,left:0,right:0,height:4,background:'rgba(0,0,0,.1)',borderRadius:'0 0 6px 6px'}}>
                              <div style={{height:'100%',background:GR.green,borderRadius:'0 0 6px 6px',width:`${(book.checkpoints.filter(c=>c.completed).length/book.checkpoints.length)*100}%`}}/>
                            </div>
                          )}
                        </div>
                        <p style={{fontSize:10,fontWeight:600,color:GR.brown,margin:0,lineHeight:1.3,display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical',overflow:'hidden'}}>{book.title}</p>
                        <p style={{fontSize:9,color:'#aaa',margin:'2px 0 0',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{book.authors?.[0]||''}</p>
                        {viewingUserBooks.tab==='reading'&&book.checkpoints?.length>0&&(
                          <p style={{fontSize:9,color:GR.greenDark,margin:'2px 0 0',fontWeight:600}}>
                            {book.checkpoints.filter(c=>c.completed).length}/{book.checkpoints.length} {lang==='es'?'días':'days'}
                          </p>
                        )}
                      </div>
                    ))}
      ...
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        @media(max-width:767px){
          /* Header más compacto */
          header > div { height:46px !important; gap:8px !important; padding:0 10px !important; }

          /* Ocultar sub-tabs en mobile — se navega por nav inferior */
          header > div + div { display:none !important; }

          /* Nav inferior más grande y cómodo en mobile */
          nav.sb-bottom-nav button { padding:10px 0 12px !important; gap:4px !important; }
          nav.sb-bottom-nav button svg { width:22px !important; height:22px !important; }
          nav.sb-bottom-nav button span { font-size:9px !important; }

          /* Cards de feed sin padding lateral excesivo */
          article { margin-left:0 !important; margin-right:0 !important; }

          /* Posts: texto de cita más legible en pantallas pequeñas */
          article blockquote, article .sb-quote {
            font-size:14px !important;
            padding:12px 14px !important;
          }

          /* Modales full-width en mobile */
          .sb-modal-inner { max-width:100% !important; border-radius:16px 16px 0 0 !important; margin-top:auto !important; max-height:95vh !important; }

          /* Grid stats 2 columnas en mobile */
          .sb-stats-grid { grid-template-columns:1fr 1fr !important; }

          /* Búsqueda: barra full width */
          .sb-search-bar { flex-wrap:wrap !important; }
          .sb-search-bar input { width:100% !important; }

          /* Portadas de libro: tamaño adecuado en mobile */
          .sb-book-cover-sm { width:52px !important; height:78px !important; }
          .sb-book-cover-md { width:72px !important; height:108px !important; }
        }

        /* Safe area para iPhone con notch */
        @supports(padding:max(0px)){
          nav.sb-bottom-nav { padding-bottom:max(10px, env(safe-area-inset-bottom)) !important; }
          header { padding-top:max(0px, env(safe-area-inset-top)) !important; }
        }
      `}</style>
    </div>
  );
