...
const i18n = {
  es: {
    ...
    total_chapters_count: "capítulos totales",
    chapters_read: "capítulos leídos",
    chapter: "Capítulo",
    read_chapter: "Marcar como leído",
  ...
  en: {
    ...
    total_chapters_count: "total chapters",
    chapters_read: "chapters read",
    chapter: "Chapter",
    read_chapter: "Mark as read",
...
// --- COMPONENTE PARA EL PLAN DE LA BIBLIA ---
const BibleReadingPlan = ({ book, onClose, t, theme, user, onUpdate }) => {
  ...
  const toggleChapter = async (chapterIndex) => {
    const newReadChapters = readChapters.includes(chapterIndex)
      ? readChapters.filter(i => i !== chapterIndex)
      : [...readChapters, chapterIndex];
    
    setReadChapters(newReadChapters);
    
    // Actualizar en Firebase
    await updateDoc(doc(db, 'users', user.uid, 'myBooks', book.bookId), {
      readChapters: newReadChapters,
      currentChapter: currentChapter
    });
    
    // Si se completaron todos los capítulos, marcar como leído
    if (newReadChapters.length === chapters.length) {
      await updateDoc(doc(db, 'users', user.uid, 'myBooks', book.bookId), {
        status: 'read',
      ...
      await updateDoc(doc(db, 'profiles', user.uid), { 
        ...
        readBooksList: arrayUnion(book.bookId)
      });
    }
  ...
  const setCurrentChapterHandler = async (chapterIndex) => {
  ...
  return (
    <div className={`fixed inset-0 z-[400] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in`}>
      <div className={`${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-white'} w-full max-w-lg h-[85vh] rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95`}>
        <div className={`relative p-6 ${theme === 'dark' ? 'bg-indigo-800' : theme === 'sunset' ? 'bg-orange-500' : 'bg-indigo-600'} flex-shrink-0`}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-black text-white">{t.bible}</h2>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-bold">{t.reading_progress}</span>
            ...
            <p className="text-xs text-slate-500 mt-2">
              {readChapters.length} {t.chapters_read} / {chapters.length} {t.total_chapters_count}
            </p>
          </div>
          
          <div className="mb-6">
            <label className="text-xs font-bold mb-2 block">{t.current_chapter}</label>
            <input 
              type="number" 
              value={currentChapter}
              onChange={(e) => setCurrentChapterHandler(parseInt(e.target.value) || 1)}
              min="1"
              max={chapters.length}
            ...
            {chapters.map((chap, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <button
                    onClick={() => setCurrentChapterHandler(chapterNumber)}
                  ...
                  >
                    {isCurrent ? t.current_day : t.set_current_day}
                  </button>
                </div>
              );
            })}
...
export default function App() {
  ...
  // --- FUNCIÓN PARA INICIAR PLAN DE LECTURA DE LA BIBLIA COMO UN SOLO LIBRO ---
  const startBibleReadingPlan = async () => {
    const totalChapters = bibleBooks.reduce((acc, book) => acc + book.chapters.length, 0);
    ...
    const bibleBookAsPlanningBook = {
      ...
      isBible: true,
      bibleBooks: bibleBooks // Guardamos la estructura completa
    };
    
    // Agregar a la biblioteca si no existe
    const existingBible = myBooks.find(b => b.bookId === 'bible-complete');
    if (!existingBible) {
      await handleAddBook(bibleBookAsPlanningBook, 'reading', false, true);
    }
    
    // Abrir el plan de la Biblia
    setBibleBook(bibleBookAsPlanningBook);
    setShowBiblePlan(true);
  };
  ...
  const rateBook = async (bookId, rating) => {
    ...
    // También guardar en reseñas globales
  ...
  const handleAddBook = async (book, status, isFav = false, addToLibrary = false) => {
    ...
    if (book.specialBible || book.isBible) {
      const info = {
        bookId,
        ...
        dislikes: 0,
        isBible: true,
        readChapters: [], // Para guardar los capítulos leídos
        currentChapter: 1 // Capítulo actual
      };
  ...
  return (
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} pb-24 font-sans overflow-x-hidden selection:bg-indigo-200 selection:text-white`}>
      ...
      {/* MODAL DEL PLAN DE LA BIBLIA */}
      {showBiblePlan && bibleBook && (
        <BibleReadingPlan 
          book={bibleBook}
          ...
          onUpdate={() => {
            // Recargar libros
            if (user) {
              const unsub = onSnapshot(collection(db, 'users', user.uid, 'myBooks'), (s) => {
                const books = s.docs.map(d => d.data());
                setMyBooks(books);
              });
              return () => unsub();
        ...
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className={`${themeClasses.card} w-full max-w-lg h-[85vh] rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95`}>
            <div className={`relative h-48 ${theme === 'dark' ? 'bg-indigo-800' : theme === 'sunset' ? 'bg-orange-500' : 'bg-indigo-600'} flex-shrink-0`}>
              {viewingBook.specialBible || viewingBook.isBible ? (
                <img 
                  src="/biblia.png" 
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-16 scrollbar-hide">
              <div className="flex items-start justify-between">
                <h2 
                  className="font-black text-2xl leading-tight cursor-pointer hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors flex-1"
                  onClick={() => handleViewBookDetails(viewingBook)}
                >
                  {viewingBook.volumeInfo?.title || viewingBook.title}
                </h2>
                <button 
                  onClick={() => toggleFavorite(viewingBook.id || viewingBook.bookId)}
                  className={`p-2 rounded-full ${
                    myBooks.find(b => b.bookId === (viewingBook.id || viewingBook.bookId))?.isFavorite
                      ? 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-300'
                      : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-300'
                  }`}
                >
                  <Star size={20} className={myBooks.find(b => b.bookId === (viewingBook.id || viewingBook.bookId))?.isFavorite ? 'fill-yellow-400' : ''} />
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMyBooks.map(book => (
                <div key={book.bookId} className={`${themeClasses.card} p-6 rounded-[2.5rem] border ${themeClasses.border} space-y-4 relative`}>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        >
                          <Star size={18} className={book.isFavorite ? 'fill-yellow-400' : ''} />
                        </button>
                  ...
                  {book.isBible && book.readChapters && (
                    <div className="mt-4">
                      ...
                      <button
                        ...
                        className="w-full mt-3 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold"
                      >
                        {t.reading_plan}
                      </button>
                    </div>
                  ...
                  {book.status === 'reading' && !book.isBible && book.checkpoints && (
                    <div className="mt-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-bold">{t.reading_progress}</span>
                        <span className="text-xs font-bold text-indigo-600">
                  ...
                  <div className="flex items-center justify-between pt-2">
                    ...
                    {book.status !== 'reading' && !book.isBible && (
                      <button 
                        onClick={() => {
                    ...
                    {book.isBible && (
                      <button 
                        ...
                        className="flex items-center gap-1 text-xs font-bold text-indigo-600"
                      >
                        <img src="/biblia.png" className="w-4 h-4 object-contain" />
                        {t.reading_plan}
                      </button>
                    )}
                  </div>
                    ...
                    <div className="flex-1">
                      ...
                      {book.isBible && book.readChapters && (
                        <div className="mt-2">
                          ...
                          <div className="h-1 bg-slate-200 dark:bg-gray-600 rounded-full mt-1 overflow-hidden">
                            <div className="h-full bg-indigo-500" style={{ width: `${(book.readChapters.length / 1189) * 100}%` }} />
                          </div>
                        </div>
                      )}
                      
                      {book.status === 'reading' && book.checkpoints && !book.isBible && (
