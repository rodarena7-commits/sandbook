  ...
  return (
    <div className="w-full max-w-xs space-y-3">
      ...
      <button onClick={onGoogle} disabled={!!loading}
        className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 rounded-2xl px-5 py-3.5 font-semibold text-slate-700 shadow-sm hover:shadow-md hover:bg-slate-50 transition-all disabled:opacity-50">
        {loading === 'google' ? <Loader2 size={20} className="animate-spin" /> : (
...
export default function LoginScreen() {
  ...
  async function handleAnon() {
    ...
    catch { setError('No se pudo continuar como invitado.') }
    setLoading(null)
  }

  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-50 flex flex-col items-center justify-center px-6">
        ...
        <div className="w-20 h-20 rounded-3xl overflow-hidden shadow-xl mb-4">
          <img src={logoSrc} alt="Sandbook" className="w-full h-full object-cover"
            onError={e => {
              ...
              e.target.parentElement.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>'
            }}
          />
        </div>
