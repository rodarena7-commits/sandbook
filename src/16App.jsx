...
export default function App() {
  ...
  // Estado para Google Lens/search por imagen
  const [showGoogleLens, setShowGoogleLens] = useState(false);
  const [googleLensImage, setGoogleLensImage] = useState(null);
  ...
  // Función para buscar libros con Google Lens (simulada)
  const searchWithGoogleLens = async (imageFile) => {
    if (!imageFile) return;
    
    setIsSearchingByImage(true);
    
    try {
      // En un entorno real, aquí se enviaría la imagen a Google Lens API
      // Por ahora simulamos una búsqueda con una consulta genérica
      
      // Primero convertimos la imagen a base64 para mostrarla
      const reader = new FileReader();
      reader.onloadend = async () => {
        setGoogleLensImage(reader.result);
        
        // Simular búsqueda de libros relacionados
        const simulatedResults = [
          {
            id: 'simulated1',
            volumeInfo: {
              title: 'Libro encontrado por imagen',
              authors: ['Autor Desconocido'],
              description: 'Este libro fue identificado mediante búsqueda por imagen.',
              imageLinks: {
                thumbnail: 'https://via.placeholder.com/150x200?text=LIBRO+IDENTIFICADO'
              }
            }
          }
        ];
        
        // También hacemos una búsqueda real en Google Books con términos genéricos
        let url = `https://www.googleapis.com/books/v1/volumes?q=book&maxResults=10`;
        if (GOOGLE_BOOKS_API_KEY) url += `&key=${GOOGLE_BOOKS_API_KEY}`;
        ...
        if (data.items) {
          setGoogleLensResults(data.items);
        } else {
          setGoogleLensResults(simulatedResults);
        }
      ...
      console.error("Error en búsqueda por imagen:", err);
      setIsSearchingByImage(false);
      
      // Mostrar resultados simulados en caso de error
      setGoogleLensResults([
        {
          id: 'error1',
          volumeInfo: {
            title: 'Intenta buscar manualmente',
            authors: ['Sistema'],
            description: 'No se pudieron identificar libros con la imagen. Intenta buscar manualmente.',
            imageLinks: {
              thumbnail: 'https://via.placeholder.com/150x200?text=NO+ENCONTRADO'
            }
          }
        }
      ]);
  ...
  // Función para manejar like/dislike de libros
  ...
  const sendFriendRequest = async (targetId, targetName) => {
    ...
    await addDoc(collection(db, 'notifications'), {
      userId: targetId,
      type: 'friend_request',
      title: lang === 'es' ? 'Nueva solicitud de amistad' : 'New friend request',
      message: lang === 'es' 
  ...
  const acceptFriendRequest = async (requestId, senderId, senderName) => {
    ...
    await addDoc(collection(db, 'notifications'), {
      ...
      title: lang === 'es' ? 'Solicitud de amistad aceptada' : 'Friend request accepted',
      message: lang === 'es' 
  ...
  // Función para manejar carga de imagen para Google Lens
  const handleImageUploadForLens = (e) => {
    const file = e.target.files[0];
    if (file) {
      searchWithGoogleLens(file);
    }
  };
  ...
  const takePhoto = () => {
    ...
    // Por ahora simulamos con un input de archivo
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = 'image/*';
    input.capture = 'environment'; // Para móviles, usa cámara trasera
    input.onchange = (e) => {
      if (e.target.files[0]) {
        searchWithGoogleLens(e.target.files[0]);
      }
    };
    input.click();
  };
    ...
    <div className={`min-h-screen ${themeClasses.bg} ${themeClasses.text} pb-24 font-sans overflow-x-hidden selection:bg-indigo-200 selection:text-white`}>
      ...
      <header className={`sticky top-0 z-50 ${theme === 'dark' ? 'bg-gray-800/90' : theme === 'sunset' ? 'bg-orange-100/90' : 'bg-white/90'} backdrop-blur-md border-b ${themeClasses.border} px-6 py-4 flex items-center justify-between shadow-sm`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => {setActiveTab('profile'); setSelectedUserProfile(null); setShowWriters(false); setShowFriendsSection(false);}} className={`flex items-center gap-2 ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : theme === 'sunset' ? 'bg-amber-100 border-amber-200' : 'bg-slate-100 border-slate-200'} p-1 rounded-full border pr-3`}>
      ...
      {/* MODAL GOOGLE LENS / BÚSQUEDA POR IMAGEN */}
      {showGoogleLens && (
        <div className="fixed inset-0 z-[500] bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in">
          <div className={`${themeClasses.card} w-full max-w-md rounded-[3rem] overflow-hidden flex flex-col shadow-2xl animate-in zoom-in-95 max-h-[90vh]`}>
            <div className={`relative p-6 ${theme === 'dark' ? 'bg-green-800' : theme === 'sunset' ? 'bg-green-500' : 'bg-green-600'} flex-shrink-0`}>
              <h2 className="text-xl font-black text-white text-center">{t.google_lens_search}</h2>
              <button onClick={() => {setShowGoogleLens(false); setGoogleLensImage(null); setGoogleLensResults([]);}} className="absolute top-6 right-6 p-2 bg-white/20 text-white rounded-full">
                <X size={24}/>
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6">
              {!googleLensImage ? (
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <button 
                      onClick={takePhoto}
                      className={`flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border-2 border-dashed ${theme === 'dark' ? 'border-gray-600 hover:border-green-500' : theme === 'sunset' ? 'border-amber-300 hover:border-green-500' : 'border-slate-200 hover:border-green-500'} transition-colors`}
                    >
                      <Camera size={32} className="text-green-500" />
                      <span className="text-sm font-bold">{t.take_photo}</span>
                    </button>
                    
                    <label className={`flex flex-col items-center justify-center gap-3 p-6 rounded-2xl border-2 border-dashed ${theme === 'dark' ? 'border-gray-600 hover:border-green-500' : theme === 'sunset' ? 'border-amber-300 hover:border-green-500' : 'border-slate-200 hover:border-green-500'} transition-colors cursor-pointer`}>
                      <Upload size={32} className="text-green-500" />
                      <span className="text-sm font-bold">{t.upload_image}</span>
                      <input 
                        type="file" 
                        accept="image/*" 
                        onChange={handleImageUploadForLens} 
                        className="hidden" 
                      />
                    </label>
                  </div>
                ...
                <div className="space-y-6">
                  <div className="text-center">
                    <img src={googleLensImage} className="max-h-48 mx-auto rounded-2xl shadow-lg" alt="Imagen cargada" />
                  </div>
                  ...
                  {isSearchingByImage ? (
                    <div className="text-center py-8">
                      ...
                      <p className="mt-4 text-sm text-slate-600 dark:text-gray-300">
                        {lang === 'es' ? 'Buscando libros...' : 'Searching for books...'}
                      </p>
                    </div>
                  ) : googleLensResults.length > 0 ? (
                    <div className="space-y-4">
                      <h3 className="text-sm font-bold">{lang === 'es' ? 'Resultados encontrados:' : 'Results found:'}</h3>
                      {googleLensResults.slice(0, 5).map((book, idx) => (
                        <div key={idx} className={`${theme === 'dark' ? 'bg-gray-800' : theme === 'sunset' ? 'bg-amber-50' : 'bg-white'} p-4 rounded-2xl border ${themeClasses.border} flex gap-4`}>
                          <img 
                            src={book.volumeInfo?.imageLinks?.thumbnail || 'https://via.placeholder.com/80x120?text=Libro'} 
                            className="w-16 h-24 object-cover rounded-lg" 
                          />
                          <div className="flex-1">
                            <h4 className="text-sm font-bold line-clamp-2">{book.volumeInfo?.title || 'Libro no identificado'}</h4>
                            ...
                            <div className="flex gap-2 mt-3">
                              ...
                              <button 
                                onClick={() => searchWithGoogle(book.volumeInfo?.title || '')}
                    ...
                    <div className="text-center py-8">
                      ...
                      <button 
                        onClick={() => searchWithGoogle('libro')}
          ...
          <>
            ...
            {activeTab === 'library' && (
              <div className="space-y-6 animate-in fade-in">
                ...
                {/* Filtros (6 secciones como solicitado - CORREGIDO: "want" ahora muestra "Pendiente") */}
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                  {['all', 'read', 'in_plan', 'want', 'favorite', 'in_library'].map(type => (
                    <button 
                      key={type} 
                        ...
                        <div className="flex gap-4">
                          ...
                          <div className="flex-1 flex flex-col justify-center">
                            ...
                            <div className="flex items-center justify-between bg-slate-50 dark:bg-gray-800 p-2 rounded-2xl border border-slate-100 dark:border-gray-700 mt-3">
                              ...
                              <div 
                                onClick={() => handleAddBook(book, book.status === 'read' ? 'want' : 'read', book.isFavorite, book.inLibrary)} 
            ...
            {/* BUSCADOR LIBROS CON GOOGLE LENS */}
            {activeTab === 'search' && (
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className={`${themeClasses.card} p-6 rounded-[2.5rem] border ${themeClasses.border} shadow-sm space-y-4`}>
                  ...
                  {/* Botón de Google Lens/Búsqueda por imagen */}
                  <button 
                    onClick={() => setShowGoogleLens(true)}
                    className={`w-full flex items-center justify-center gap-2 py-4 rounded-2xl font-bold text-sm shadow-lg active:scale-95 transition-all ${
                      theme === 'dark' 
                          ...
                          <div className="relative">
                            ...
                            {!hasThumbnail && (
                              <div className="absolute inset-0 flex items-center justify-center">
                                <button 
                                  onClick={() => searchWithGoogle(book.volumeInfo?.title)}
                                  className="px-3 py-1.5 bg-red-500 hover:bg-red-600 text-white rounded-lg text-xs font-bold flex items-center gap-1"
                                >
                                  <Globe size={12} />
                                  {t.search_with_google}
                                </button>
                          ...
                          <div className="flex-1 flex flex-col">
                            ...
                            <div className="mt-auto space-y-2">
                              ...
                              <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
                                ...
                                {/* Botón para buscar en Google si no hay imagen */}
                                {!hasThumbnail && (
                                  <button 
                                    onClick={() => searchWithGoogle(book.volumeInfo?.title)}
                                    className="p-2 bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-300 rounded-xl active:scale-95"
                                    title={t.search_with_google}
                                  >
                                    <Globe size={14}/>
                                  </button>
                                )}
      ...
      <nav className={`fixed bottom-0 left-0 right-0 backdrop-blur-lg border-t ${themeClasses.border} px-8 py-4 flex justify-between items-center z-40 shadow-2xl ${
        ...
        {[{id:'library',icon:Layout,l:t.library},{id:'search',icon:Search,l:t.plan},{id:'social',icon:Globe,l:t.social},{id:'profile',icon:User,l:t.profile}].map(t_nav => (
          <button 
            key={t_nav.id} 
            onClick={() => {setActiveTab(t_nav.id); setSelectedUserProfile(null); setShowWriters(false); setShowFriendsSection(false);}} 
